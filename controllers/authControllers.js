...
const resetPassword = async (req, res) => {
    try {
        ...
        // Check if target user is a tech person
        const isTargetTech = (
            String(user.email || '').toLowerCase().includes('tech') ||
            String(user.role || '').toLowerCase().includes('tech')
        );
        if (isTargetTech) {
            return res.status(403).json({
                message: 'Access denied. Password of a tech person cannot be reset via token.'
            });
        }
