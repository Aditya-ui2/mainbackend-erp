...
const deepSearchSharePoint = async (req, res) => {
    try {
        ...
        if (results.length > 0) {
            const filesToSync = results.map(r => ({
                id: r.sharePointId,
                name: r.name,
                fileType: r.fileType,
                size: r.size,
            ...
            await saveResumesToDB(filesToSync, 'sharepoint');
        }

        // Map SharePoint results to existing database records now that they are synced
...
const streamResume = async (req, res) => {
    try {
        ...
        // Fallback: Check SharePointCandidate table
        let spCandidate = null;
        if (!resume) {
            spCandidate = await SharePointCandidate.findOne({
                where: {
                    ...
                    if (candidateList) {
                        ...
                        if (attachments && attachments.length > 0) {
                            ...
                            if (downloadUrl) {
                                ...
                                const spResponse = await axios.get(downloadUrl, { responseType: 'stream' });
                                
                                const isDownload = req.query.download === 'true';
                                res.setHeader('Content-Type', spResponse.headers['content-type'] || 'application/pdf');
                                res.setHeader('Content-Disposition', `${isDownload ? 'attachment' : 'inline'}; filename="${firstAttachment.name}"`);
                                
                                return spResponse.data.pipe(res);
                            }
                        }
                    }
                } catch (spErr) {
                    console.warn('SharePoint candidate stream failed:', spErr.message);
                }
        ...
        // Fallback: Check Candidate table
        let candidate = null;
        if (!resume && !spCandidate && isUUID) {
            candidate = await Candidate.findByPk(id);
            if (candidate && candidate.cvUrl) {
                const fs = require('fs');
                const path = require('path');
                const absolutePath = path.join(__dirname, '..', candidate.cvUrl);
                if (fs.existsSync(absolutePath)) {
