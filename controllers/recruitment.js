...
const { TeamLeader, Client, RecruitmentPosition, Candidate, Interview, OfferTemplate, DepartmentTask, ResumeBank, DepartmentTeam, DepartmentNote, SharePointCandidate, DailyReport, sequelize } = require('../models/sequelizeModels');
const fs = require('fs');
...
const normalizePriority = (value = '') => {
    ...
    if (val.toLowerCase() === 'critical') return 'Urgent';
    const match = ['low', 'medium', 'high', 'urgent'].indexOf(val.toLowerCase());
    if (match !== -1) {
        return ['Low', 'Medium', 'High', 'Urgent'][match];
    }
    return 'Medium';
};
...
const normalizePositionStatus = (value = '', priority = '') => {
    ...
    if (priority === 'Urgent' || priority === 'Critical') return 'Open';
    return 'Open';
};
...
const getRecruitmentClients = async (req, res) => {
    try {
        ...
        // If the logged in user is a SuperAdmin, Admin, or TeamLeader (Recruitment Head/TL),
        // let them see all clients, because they manage all recruitment tasks/positions.
        const userRole = String(req.user?.role || req.user?.userType || '').toLowerCase();
        const isStaffOrAdmin = ['superadmin', 'super_admin', 'admin', 'administrator', 'teamleader', 'team_leader', 'tl', 'recruitmenthead', 'recruitment head'].some(r => userRole.includes(r) || userRole === r);

        let whereClause = {};

        if (!isStaffOrAdmin) {
            // Find all possible name variants for the user to match assignKAM case-insensitively
            const names = new Set();
            if (userName) {
                ...
                // Add first name
                const firstName = cleaned.split(' ')[0].split('(')[0].trim();
                if (firstName) names.add(firstName);
            }

            if (userEmail) {
                const { DepartmentTeam, Employee, TeamLeader } = require('../models/sequelizeModels');
                const dtMember = await DepartmentTeam.findOne({ where: { email: { [Op.iLike]: userEmail.trim() } } });
                if (dtMember && dtMember.name) {
                    const cleaned = dtMember.name.trim();
                    names.add(cleaned);
                    const firstName = cleaned.split(' ')[0].split('(')[0].trim();
                    if (firstName) names.add(firstName);
                }
                const emp = await Employee.findOne({ where: { email: { [Op.iLike]: userEmail.trim() } } });
                if (emp && emp.name) {
                    const cleaned = emp.name.trim();
                    names.add(cleaned);
                    const firstName = cleaned.split(' ')[0].split('(')[0].trim();
                    if (firstName) names.add(firstName);
                }
                const tl = await TeamLeader.findOne({ where: { email: { [Op.iLike]: userEmail.trim() } } });
                if (tl && tl.name) {
                    const cleaned = tl.name.trim();
                    names.add(cleaned);
                    const firstName = cleaned.split(' ')[0].split('(')[0].trim();
                    if (firstName) names.add(firstName);
                }
            ...
            const nameArray = Array.from(names).filter(Boolean);
            ...
            if (nameArray.length > 0) {
                ...
                // If we couldn't resolve any name, return empty
...
const getKamsWithRecruitment = async (req, res) => {
    try {
        ...
        if (req.user?.id && req.query.excludeSelf === 'true') {
            excludedRecruitmentIds.push(req.user.id);
        }
        ...
        const kams = await DepartmentTeam.findAll({
            where: {
                [Op.and]: [
                    sequelize.where(
                        sequelize.cast(sequelize.col('department'), 'TEXT'),
                        { [Op.iLike]: '%recruitment%' }
                    ),
                    {
                        id: { [Op.notIn]: excludedRecruitmentIds },
                        role: { [Op.notIn]: ['Super Admin', 'SuperAdmin'] }
                    }
                ]
            },
        ...
        const kamsWithStats = await Promise.all(kams.map(async (kam) => {
            try {
                ...
                return {
                    ...k,
                    avatar: kam.avatar || kam.picture || kam.name.split(' ').map(n => n[0]).join('').substring(0, 2).toUpperCase(),
                    picture: kam.picture || kam.avatar || null,
                    stats: {
                ...
                const fallbackJson = kam.toJSON ? kam.toJSON() : kam;
                return {
                    ...fallbackJson,
                    avatar: kam.avatar || kam.picture || kam.name.split(' ').map(n => n[0]).join('').substring(0, 2).toUpperCase(),
                    picture: kam.picture || kam.avatar || null,
...
const addCandidate = async (req, res) => {
    ...
    try {
        ...
        let { name, email, phone, positionId, clientId, skills, experience, currentSalary, expectedSalary, notes, location, noticePeriod, stage, pipelineStatus, rating, source, joiningDate, offeredCTC, status, currentJobRole, currentDesignation, hometown, preferredLocation, relevantExperience, highestQualification, reasonForLeaving } = req.body;
        ...
        try {
            candidate = await Candidate.create({
                ...
                currentJobRole, currentDesignation, hometown, preferredLocation,
                relevantExperience, highestQualification, reasonForLeaving
            });
        } catch (fkError) {
...
const updateCandidateStatus = async (req, res) => {
    try {
        ...
        if (req.body.joiningStatus) updateData.joiningStatus = req.body.joiningStatus;
...
const updateCandidate = async (req, res) => {
    ...
    try {
        const { id } = req.params;
        let { name, email, phone, positionId, clientId, skills, experience, currentSalary, expectedSalary, notes, location, noticePeriod, stage, pipelineStatus, rating, source, currentJobRole, currentDesignation, hometown, preferredLocation, relevantExperience, highestQualification, reasonForLeaving } = req.body;
...
const getRecruitmentStats = async (req, res) => {
    ...
    try {
        ...
        let positionMetrics;
        try {
            let positions = await RecruitmentPosition.findAll({
                where: {
                    ...dateFilter,
                    ...(clientId ? { clientId } : {})
                },
                include: [{ model: Client, as: 'client', attributes: ['companyName', 'name', 'status'] }]
            });

            // Filter out positions of deleted or Inactive/Rejected clients
            positions = positions.filter(pos => {
                if (pos.clientId) {
                    if (!pos.client) return false;
                    if (['Inactive', 'Rejected'].includes(pos.client.status)) return false;
                }
                return true;
            }).slice(0, 10);
        ...
        let clientMetrics;
        try {
            let allPositions = await RecruitmentPosition.findAll({
                where: {
                    ...dateFilter,
                    ...(clientId ? { clientId } : {})
                },
...
const getAllPositions = async (req, res) => {
    ...
    try {
        ...
        // Map them back
        let positionsWithData = positions.map(pos => ({
            ...pos,
        ...
        // Filter out positions for clients that are deleted or on hold (Inactive/Rejected)
        positionsWithData = positionsWithData.filter(pos => {
            if (pos.clientId) {
                if (!pos.client) return false; // Client was deleted
                if (['Inactive', 'Rejected'].includes(pos.client.status)) return false; // Client is on hold
            }
            return true;
        });

        // Get candidate counts
        const positionIds = positionsWithData.map(p => p.id);
...
const getOfferCandidatesSuggestions = async (req, res) => {
    try {
        ...
        let whereClause;
        if (!trimmedSearch) {
            // Default: return candidates whose status is Shortlisted or Selected
            whereClause = {
                status: { [Op.in]: ['Shortlisted', 'Selected'] },
...
const createRequest = async (req, res) => {
    try {
        ...
        const position = await RecruitmentPosition.create({
            ...
            status: status || 'Open', priority: normalizePriority(priority),
...
const getMyPerformanceStats = async (req, res) => {
    try {
        ...
        const isHead = ['department head', 'admin', 'recruitment head', 'recruitmenthead', 'team leader', 'teamleader', 'tl'].includes(userRole) || req.user.id === '1e2cfcc6-a91d-4037-95db-88cb7b04d376';
...
const deleteOffersBulk = async (req, res) => {
    try {
        const { offerIds } = req.body;
        if (!offerIds || !Array.isArray(offerIds)) {
            return res.status(400).json({ success: false, message: 'offerIds must be an array' });
        }
        ...
        res.status(200).json({ success: true, message: 'Offers deleted successfully' });
    } catch (error) {
        console.error('Error deleting offers:', error);
        res.status(500).json({ success: false, message: 'Failed to delete offers', error: error.message });
    }
};
...
const verifyCandidateKYC = async (req, res) => {
    try {
        ...
        // BGV and document verification status linking
        const mandatoryDocTypes = ['pan_front', 'pan_back', 'aadhar_front', 'aadhar_back', 'marksheet_10', 'marksheet_12'];
        let hasRejected = false;
        let allMandatoryVerified = true;
        let hasUploadedAny = false;

        for (const key of Object.keys(kycDocuments)) {
            if (kycDocuments[key] && kycDocuments[key].url) {
                hasUploadedAny = true;
                if (kycDocuments[key].verified === false) {
                    hasRejected = true;
                }
            }
        }

        for (const reqKey of mandatoryDocTypes) {
            if (!kycDocuments[reqKey] || !kycDocuments[reqKey].url || kycDocuments[reqKey].verified !== true) {
                allMandatoryVerified = false;
            }
        }

        if (hasRejected) {
            candidate.bgvStatus = 'Re-upload Requested';
        } else if (allMandatoryVerified) {
            candidate.bgvStatus = 'Verified';
        } else if (hasUploadedAny) {
            candidate.bgvStatus = 'Pending Review';
        }
...
const submitCandidateKYC = async (req, res) => {
    try {
        ...
        res.status(200).json({
            ...
            data: { bgvStatus: 'Document Submitted' }
        });
    } catch (error) {
