...
// Configure multer for general documents / policies
...
const uploadDoc = multer({ 
    ...
    fileFilter: (req, file, cb) => {
        const allowed = ['.xlsx', '.xls', '.pdf', '.doc', '.docx', '.png', '.jpg', '.jpeg', '.gif'];
        const ext = path.extname(file.originalname).toLowerCase();
        if (allowed.includes(ext)) cb(null, true);
        else cb(new Error('Invalid file type. Only Excel, PDF, Word and Images are allowed.'));
    }
});
 ...
 */
router.post('/members', verifyAuthToken, authorize('superadmin', 'manager'), addTeamMember);
 ...
 */
router.put('/members/:id', verifyAuthToken, authorize('superadmin', 'manager'), updateTeamMember);
 ...
 */
router.delete('/members/:id', verifyAuthToken, authorize('superadmin', 'manager'), deleteTeamMember);
...
router.post('/documents', verifyAuthToken, uploadDoc.single('file'), mf.uploadDocument);
router.put('/documents/:id', verifyAuthToken, uploadDoc.single('file'), mf.updateDocument);
...
// HR/Department-specific views
router.get('/fnf', verifyAuthToken, getFnFList);
router.get('/compliance', verifyAuthToken, getDeptCompliance);
router.get('/engagement', verifyAuthToken, getDeptEngagement);
router.get('/policies', verifyAuthToken, getDeptPolicies);
router.get('/productivity', verifyAuthToken, getDeptProductivity);
router.get('/tasks-by-client', verifyAuthToken, getDeptTasksByClient);
