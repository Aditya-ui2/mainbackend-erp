...
const Invoice = sequelize.define('Invoice', {
    ...
    notes: { type: DataTypes.TEXT },
    invoiceFileName: { type: DataTypes.STRING },
    invoiceFileData: { type: DataTypes.TEXT },
...
Lead.init({
    ...
    status: {
        type: DataTypes.ENUM('Open', 'New', 'Qualified', 'In Progress', 'Follow Up', 'Proposal', 'Negotiation', 'Converted', 'Lost', 'Active', 'Inactive'),
        defaultValue: 'Open'
    },
    segment: {
        type: DataTypes.STRING,
...
const PaymentRecord = sequelize.define('PaymentRecord', {
    ...
    amountReceived: { type: DataTypes.DECIMAL(15, 2), allowNull: false },
    dateReceived: { type: DataTypes.DATEONLY, allowNull: false },
...
const PaymentRequest = sequelize.define('PaymentRequest', {
    ...
    paymentSource: { type: DataTypes.STRING, defaultValue: 'Client Side' },
...
const Campaign = sequelize.define('Campaign', {
    id: { type: DataTypes.UUID, defaultValue: DataTypes.UUIDV4, primaryKey: true },
    name: { type: DataTypes.STRING, allowNull: false },
    type: { type: DataTypes.STRING, defaultValue: 'Email Campaigns' },
    ownerName: { type: DataTypes.STRING },
    targetReach: { type: DataTypes.STRING, defaultValue: '0' },
    engagement: { type: DataTypes.STRING, defaultValue: '0%' },
    budget: { type: DataTypes.STRING, defaultValue: '₹0' },
    duration: { type: DataTypes.STRING },
    roas: { type: DataTypes.STRING },
    status: { type: DataTypes.STRING, defaultValue: 'Active' },
    notes: { type: DataTypes.TEXT },
...
const ClientReview = sequelize.define('ClientReview', {
    ...
    reviewMonth: { type: DataTypes.STRING, allowNull: false },
    date: { type: DataTypes.STRING },
    reviewDate: { type: DataTypes.STRING },
    nextReviewDate: { type: DataTypes.STRING },
    createdBy: { type: DataTypes.UUID, allowNull: true },
    overallSatisfaction: { type: DataTypes.INTEGER, defaultValue: 5 },
    spocCommunication: { type: DataTypes.INTEGER, defaultValue: 5 },
    candidateProfileQuality: { type: DataTypes.INTEGER, defaultValue: 5 },
    sourcingSpeed: { type: DataTypes.INTEGER, defaultValue: 5 },
    payrollAccuracy: { type: DataTypes.INTEGER, defaultValue: 5 },
    complianceManagement: { type: DataTypes.INTEGER, defaultValue: 5 },
    reviewType: { type: DataTypes.STRING, defaultValue: 'monthly' },
    highlights: { type: DataTypes.TEXT, defaultValue: '' },
    actionItems: { type: DataTypes.TEXT, defaultValue: '' },
}, { tableName: 'client_reviews', timestamps: true });

// ============== PROFITABILITY REPORT MODEL ==============
const ProfitabilityReport = sequelize.define('ProfitabilityReport', {
    id: { type: DataTypes.UUID, defaultValue: DataTypes.UUIDV4, primaryKey: true },
    reportNumber: { type: DataTypes.STRING, allowNull: false, unique: true },
    reportName: { type: DataTypes.STRING, allowNull: false },
    reportType: { type: DataTypes.STRING, allowNull: false }, // 'Monthly', 'Quarterly', 'Half-Month', 'Yearly'
    period: { type: DataTypes.STRING, allowNull: false },
    department: { type: DataTypes.STRING, allowNull: false, defaultValue: 'All' },
    ...
    margin: { type: DataTypes.STRING, defaultValue: '0%' },
    generatedBy: { type: DataTypes.STRING, defaultValue: 'System' },
    status: { type: DataTypes.STRING, defaultValue: 'Generated' },
    notes: { type: DataTypes.TEXT },
    details: { type: DataTypes.JSONB, defaultValue: {} } // salary, rent, misc, incomingBills, otherExpenses
}, { tableName: 'profitability_reports', timestamps: true });
...
// ============ RECRUITMENT NOTIFICATION BROADCASTERS & HOOKS ============

const notifyAllInternalUsers = async (message, type = 'message', priority = 'low') => {
    try {
        const [superAdmins, admins, teamLeaders, employees, deptTeams] = await Promise.all([
            SuperAdmin.findAll({ attributes: ['id'] }),
            Admin.findAll({ attributes: ['id'] }),
            TeamLeader.findAll({ attributes: ['id'] }),
            Employee.findAll({ attributes: ['id'] }),
            DepartmentTeam.findAll({ attributes: ['id'] })
        ]);

        const uniqueUsers = new Map();

        superAdmins.forEach(u => uniqueUsers.set(u.id, 'Admin'));
        admins.forEach(u => uniqueUsers.set(u.id, 'Admin'));
        teamLeaders.forEach(u => uniqueUsers.set(u.id, 'TeamLeader'));
        employees.forEach(u => uniqueUsers.set(u.id, 'Employee'));
        deptTeams.forEach(u => {
            let userType = 'DepartmentTeam';
            if (u.role === 'Super Admin' || u.role === 'SuperAdmin') userType = 'Admin';
            uniqueUsers.set(u.id, userType);
        });

        const notificationPromises = Array.from(uniqueUsers.entries()).map(([userId, userType]) => {
        ...
        console.log(`Successfully broadcasted notification to ${uniqueUsers.size} users.`);
    } catch (error) {
        console.error('Error broadcasting notification to internal users:', error);
    }
};

// RecruitmentPosition Hooks
RecruitmentPosition.addHook('afterCreate', async (position, options) => {
    try {
        let clientInfo = '';
        if (position.clientId) {
            const client = await Client.findByPk(position.clientId);
            if (client) {
                clientInfo = ` for client "${client.companyName || client.name}"`;
            }
        }
        const msg = `🆕 New Job Position: "${position.title}" has been created${clientInfo}.`;
        await notifyAllInternalUsers(msg, 'system', 'medium');
    } catch (error) {
        console.error('Error in RecruitmentPosition afterCreate hook:', error);
    }
});

RecruitmentPosition.addHook('afterUpdate', async (position, options) => {
    try {
        const changed = position.changed();
        if (changed && changed.length > 0) {
            let msg = '';
            if (changed.includes('status')) {
                msg = `🔄 Job Position Status Change: "${position.title}" is now "${position.status}".`;
            } else {
                msg = `🔄 Job Position Updated: "${position.title}" has been modified.`;
            }
            await notifyAllInternalUsers(msg, 'system', 'medium');
        }
    } catch (error) {
        console.error('Error in RecruitmentPosition afterUpdate hook:', error);
    }
});

// Candidate Hooks
Candidate.addHook('afterCreate', async (candidate, options) => {
    try {
        let positionInfo = '';
        if (candidate.positionId) {
            const position = await RecruitmentPosition.findByPk(candidate.positionId);
            if (position) {
                positionInfo = ` for position "${position.title}"`;
            }
        }
        const msg = `👤 New Candidate Sourced: "${candidate.name}" has been added${positionInfo}.`;
        await notifyAllInternalUsers(msg, 'system', 'medium');
    } catch (error) {
        console.error('Error in Candidate afterCreate hook:', error);
    }
});
    ...
    try {
        ...
        if (changed && changed.length > 0) {
            ...
            if (candidate.positionId) {
                const position = await RecruitmentPosition.findByPk(candidate.positionId);
                if (position) {
                    positionInfo = ` for "${position.title}"`;
                }
            }
            let msg = '';
            if (changed.includes('status') || changed.includes('stage')) {
                msg = `🔄 Candidate Update: "${candidate.name}"${positionInfo} is now "${candidate.status}" (${candidate.stage}).`;
            } else if (changed.includes('kycDocuments')) {
                msg = `📄 Candidate Document Uploaded: "${candidate.name}"${positionInfo} has uploaded documents.`;
            } else if (changed.includes('bgvStatus') && candidate.bgvStatus === 'Document Submitted') {
                msg = `📄 Candidate Document Submitted: "${candidate.name}"${positionInfo} has submitted documents for verification.`;
            } else {
                msg = `🔄 Candidate Updated: "${candidate.name}"${positionInfo}'s profile details have been modified.`;
            }
            await notifyAllInternalUsers(msg, 'system', 'medium');
        }
    } catch (error) {
        console.error('Error in Candidate afterUpdate hook:', error);
    }
});

// Interview Hooks
Interview.addHook('afterCreate', async (interview, options) => {
    try {
        ...
        const formattedDate = interview.interviewDate ? new Date(interview.interviewDate).toLocaleDateString('en-IN') : 'N/A';
        const msg = `📅 Interview Scheduled: "${interview.interviewType}" round for candidate "${candidateName}" (Position: "${positionTitle}") is scheduled for ${formattedDate} at ${interview.startTime}.`;
        await notifyAllInternalUsers(msg, 'system', 'medium');
    } catch (error) {
        console.error('Error in Interview afterCreate hook:', error);
    }
});

Interview.addHook('afterUpdate', async (interview, options) => {
    try {
        const changed = interview.changed();
        if (changed && changed.length > 0) {
            const [candidate, position] = await Promise.all([
                Candidate.findByPk(interview.candidateId),
                RecruitmentPosition.findByPk(interview.positionId)
            ...
            const positionTitle = position ? position.title : 'Unknown Position';
            let msg = '';
            if (changed.includes('evaluation') && interview.evaluation) {
                msg = `📝 Interview Evaluated: "${interview.interviewType}" feedback has been submitted for candidate "${candidateName}" (Position: "${positionTitle}") - Recommendation: "${interview.evaluation.recommendation || 'N/A'}".`;
            } else if (changed.includes('status')) {
                msg = `🔄 Interview Status Update: "${interview.interviewType}" for candidate "${candidateName}" (Position: "${positionTitle}") is now "${interview.status}".`;
            } else {
                msg = `🔄 Interview Updated: "${interview.interviewType}" for candidate "${candidateName}" (Position: "${positionTitle}") details were modified.`;
            }
            await notifyAllInternalUsers(msg, 'system', 'medium');
        }
    } catch (error) {
        console.error('Error in Interview afterUpdate hook:', error);
    }
});
