...
// Client Review Schema - for monthly client reviews
...
clientReviewSchema.index({ clientId: 1, reviewMonth: 1 }, { unique: true });
...
// Problem / Help & Support Schema
const problemSchema = new Schema({
    id: { type: String, unique: true }, // custom ID like PRB-001
    title: { type: String, required: true },
    description: { type: String, required: true },
    priority: { type: String, enum: ['Low', 'Medium', 'High', 'Urgent'], default: 'Medium' },
...
// Pre-save hook to generate PRB-xxx ID
problemSchema.pre('save', async function(next) {
    if (!this.id) {
        const count = await mongoose.model('Problem').countDocuments();
        this.id = `PRB-${String(count + 1).padStart(3, '0')}`;
    }
    next();
});
